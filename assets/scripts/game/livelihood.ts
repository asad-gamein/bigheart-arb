...
export class Livelihood extends Component {
    ...
    moveTruck(container: any | Node, truckType: LIVLI_CONTAINER_TYPE) {
        ...
        tween(container.item)
            ...
            .call(() => {
                this.setRandomTruck(container);
            })
            .to(0, { position: new Vec3(initPos!.x, initPos!.y, 0) })
            .call(() => {
                this.setItemCountInContainer(truckType, 0);
            })
            .start();
    }
    ...
    playResultVoiceOver(audioClips: any) {
        ...
        if (audioClips.length == 1) {
            tween(this.node)
                .call(() => SoundManager.getInstance().playSoundEffect(your))
                .delay(1)
                .call(() => {
            ...
            tween(this.node)
                ...
                .delay(2)
                .call(() => SoundManager.getInstance().playSoundEffect(result[0]))
                .delay(2)
                .call(() => SoundManager.getInstance().playSoundEffect(result[1]))
                .delay(2)
                .call(() => {
                    SoundManager.getInstance().playSoundEffect(sectorHelp);
                })
