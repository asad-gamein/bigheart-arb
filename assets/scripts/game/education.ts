...
export class Education extends Component {
    ...
    addRequest() {
        ...
        if (randomItemType == EDU_ITEM_TYPE.SCHOOL) {
            ...
            if (this.schoolCount == 2) this.itemType.splice(this.itemType.indexOf(EDU_ITEM_TYPE.SCHOOL), 1);
        }
    ...
    checkDragableIntersectWith(container: any | Node) {
        if (container && this.dragable.item) {
            return Intersection2D.rectRect(container.getComponent(UITransform)?.getBoundingBoxToWorld(), this.dragable.item.getComponent(UITransform)?.getBoundingBoxToWorld()!);
        } else {
            return false;
        }
    }
    ...
    getCountyName(countryType: EDU_CONTAINER_TYPE | undefined) {
        switch (countryType) {
            case EDU_CONTAINER_TYPE.ALGERIA:
                return "الجزائر";
            case EDU_CONTAINER_TYPE.EGYPT:
                return "مصر";
            case EDU_CONTAINER_TYPE.IRAQ:
                return "العراق";
            case EDU_CONTAINER_TYPE.LIBYA:
                return "ليبيا";
            case EDU_CONTAINER_TYPE.MORROCO:
                return "المغرب";
            case EDU_CONTAINER_TYPE.OMAN:
                return "عمان";
            case EDU_CONTAINER_TYPE.SAUDIARABIA:
                return "المملكة العربية السعودية";
            case EDU_CONTAINER_TYPE.SYRIA:
                return "سوريا";
            case EDU_CONTAINER_TYPE.UAE:
                return "الامارات";
            case EDU_CONTAINER_TYPE.YEMEN:
                return "اليمن";
            case EDU_CONTAINER_TYPE.PALESTINE:
                return "فلسطين";
            case EDU_CONTAINER_TYPE.PAKISTAN:
                return "باكستان";
            case EDU_CONTAINER_TYPE.INDIA:
                return "الهند";
        }
    }
    ...
    getItemNameForSummary(countryType: EDU_ITEM_TYPE | undefined) {
        switch (countryType) {
            case EDU_ITEM_TYPE.BOOK:
                return "كتاب دراسي و قرطاسية لإحدى المدارس";
            case EDU_ITEM_TYPE.COMPUTER:
                return "حاسوب لإحدى المدارس";
            case EDU_ITEM_TYPE.FURNITURE:
                return "قطعة أثاث مدرسي لإحدى المدارس";
            case EDU_ITEM_TYPE.SCHOOL:
                return "مبنى دراسي ";
        }
    }
    ...
    playAudios(index: number, audioClips: any) {
        ...
        if (index >= audioClips.length || audioClips.length == 0) {
            ...
            tween(this.node)
                .call(() => SoundManager.getInstance().playSoundEffect(audioClips[index]))
                .delay(wait)
                .call(() => this.playAudios(index + 1, audioClips))
                .start();
        }
    }
}
