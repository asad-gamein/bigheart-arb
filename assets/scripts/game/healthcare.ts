...
export class Healthcare extends Component {
    ...
    createSummary() {
        ...
        this.donationInfo!.forEach((info, index, arr) => {
            ...
            if (info.count! > 1) {
                summaryText = summaryText + `${this.getItemNameForSummary(info.type)} \u200E${info.count}`; // + "s";
            } else {
                summaryText = summaryText + `${this.getItemNameForSummary(info.type)} \u200E${info.count}`;
            }
            //summaryText = summaryText +` to hospital.`;
    ...
    getItemNameForSummary(equipmentType: HEALTH_ITEM_TYPE | undefined) {
        switch (equipmentType) {
            case HEALTH_ITEM_TYPE.BED:
                return "سرير طبي";
            case HEALTH_ITEM_TYPE.CHAIR:
                return "كرسي";
            case HEALTH_ITEM_TYPE.CLOCK:
                return "ساعة";
            case HEALTH_ITEM_TYPE.CURTAINS:
                return "ستارة";
            case HEALTH_ITEM_TYPE.MONITOR:
                return "شاشة طبية";
            case HEALTH_ITEM_TYPE.PLANT:
                return "نبتة";
            case HEALTH_ITEM_TYPE.XRAY:
                return "جهاز أشعة";
            case HEALTH_ITEM_TYPE.MEDICINE:
                return "علبة أدوية";
            case HEALTH_ITEM_TYPE.OXYGEN:
                return "اسطوانة أكسجين";
        }
    }
