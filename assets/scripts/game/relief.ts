...
export class Relief extends Component {
    ...
    checkBoxIntersectWithPlatform(box: any | Node) {
        return Intersection2D.rectRect(box.getComponent(UITransform)?.getBoundingBoxToWorld(), this.platform.getComponent(UITransform)?.getBoundingBoxToWorld()!);
    }
    ...
    onCollisionBegin(selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) {
        ...
        if (
            (selfCollider.node.type == ColliderGroup.BOX && otherCollider.node.type == ColliderGroup.PLATFORM) ||
            (selfCollider.node.type == ColliderGroup.PLATFORM && otherCollider.node.type == ColliderGroup.BOX) ||
            (selfCollider.node.type == ColliderGroup.BOX && otherCollider.node.type == ColliderGroup.BOX)
        ) {
            ...
            (selfCollider.node.type == ColliderGroup.BOX && otherCollider.node.type == ColliderGroup.BOUNDARY) ||
    ...
    allBoxesCollected() {
        ...
        this.schedule(this.moveBoxesToPlane, 0.8);
    }
    enterPlane() {
        tween(this.plane)
            .call(() => {
            ...
            .delay(0.5)
            .call(() => {
                if (this.tutorial) this.runTutorial();
